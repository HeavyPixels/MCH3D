...
The external address interface is presented as separate U and V coordinates, as this will be values calculated by the DrawLine module. To calculate the memory address, the U and V coordinates are 'swizzled', alternating bits from each to produce a square (instead of linear) addressing pattern:
```
23                     0
|                      |
